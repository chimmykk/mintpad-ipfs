  ...
  const handleFolderUpload = async (event) => {
    ...
    xhr.onload = async () => {
      ...
      if (xhr.status === 200) {
        ...
        setIpfsLink(result.ipfsLink); // Store IPFS link from response
        ...
        if (result.logs && result.logs.includes('Root CID:')) {
          ...
          const rootCidLines = logParts.filter(line => line.includes('Root CID:')); // Filter lines that contain "Root CID"
          
          if (rootCidLines.length > 1) {
            const secondRootCidLine = rootCidLines[1]; // Get the second Root CID line
            setSecondRootCid(secondRootCidLine.replace('Root CID: ', '').trim()); // Extract and set the second Root CID
          }
        }
      } else {
    ...
    <div>
      ...
      {uploadComplete && secondRootCid && (
        <div style={{ textAlign: 'center', marginTop: '20px' }}>
          <p style={{ fontSize: '16px', color: '#333' }}>
            Second Root CID: <a href={`https://uneven-lavender-harrier.myfilebase.com/ipfs/+${secondRootCid}`} target="_blank" rel="noopener noreferrer">
              {secondRootCid}
            </a>
          </p>
